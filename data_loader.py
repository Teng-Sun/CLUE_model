...
def get_loader(config, shuffle = True, ban_word_list = []):
    ...
    def collate_fn(batch):
        ...
        ban_batch_sentence_vector = []
        for sample in batch:
            ban_sentence_vector = []
            for word in sample['data'][0][3]:
                if random.random() < config.random_rate or word not in ban_word_list:
                    word_id = dataset.word2id[word]
                    ban_sentence_vector.append(dataset.pretrained_emb[word_id])
                else: 
                    word_id = dataset.word2id['<pad>']
                    ban_sentence_vector.append(dataset.pretrained_emb[word_id])
            ban_batch_sentence_vector.append(torch.stack(ban_sentence_vector))


        sentences_vector = pad_sequence(batch_sentence_vector)
        ban_sentences_vector = pad_sequence(ban_batch_sentence_vector)
        ...
        sample_data = {
            ...
            'ban_text': to_gpu(ban_sentences_vector, gpu_id = config.gpu_id),
