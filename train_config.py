...
normal_string = 'nohup python train.py --data {} --dataset_name {} --base_model {} --output_size {} --gpu_id 0'.format(data, data_name, model_name, output_size)

clip =                      [('clip', i) for i in                       [2.0, 1.5, 1.0]]
klloss =                    [('klloss', i) for i in                     [True, False]]
fusion_mode =               [('fusion_mode', i) for i in                ['sum', 'rubi', 'hm']]
tmodel_learning_rate =      [('tmodel_learning_rate', i) for i in       [1e-5, 3e-5, 6e-5]]
tmodel_weight_decay =       [('tmodel_weight_decay', i) for i in        [1e-5, 3e-5]]
...
selfmm_learning_rate =      [('selfmm_learning_rate', i) for i in       [1e-5, 3e-5, 6e-5]]
selfmm_weight_decay =       [('selfmm_weight_decay', i) for i in        [0, 1e-5]]
selfmm_H =                  [('selfmm_H', i) for i in                   [2.0, 3.0]]

new_list = [[[]], selfmm_learning_rate, tmodel_learning_rate, fusion_mode]
...
with open('{}.bash'.format(run_name), 'w+') as f:
    for index, config_list in enumerate(all_config_list):
        ...
        f.write(' >> /home/causal_model/logs/{}/{}.log \n'.format(run_name, index))